...
} from '../../../../components/ui/consultation-requests-manager';

export default async function PlatformProfessionalsPage() {
  const token = (await cookies()).get('accessToken')?.value;
  const [professionals, consultations] = await Promise.all([
    api.get<PlatformProfessional[]>('/platform-professionals/admin/all', {
      accessToken: token,
    }).catch(() => []),
    api.get<AdminConsultationRequest[]>(
      '/platform-professionals/admin/consultations',
      { accessToken: token },
    ).catch(() => []),
  ]);
  return (
    <div className="flex flex-col gap-6">
      ...
      <ConsultationRequestsManager initial={consultations} />
    </div>
  );
}
