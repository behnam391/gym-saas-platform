...
export function MembersTable({ initialMembers }: { initialMembers: ManagedMember[] }) {
  ...
  async function approveConsent(userId: string) {
    ...
    try {
      ...
      setMembers((items) => items.map((item) => item.id === userId ? { ...item, isRestricted: false, parentalConsent: item.parentalConsent ? { ...item.parentalConsent, status: 'APPROVED' } : null } : item));
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[920px] text-right text-sm">
          ...
          <tbody>{members.map((member) => {
            ...
            const membership = member.memberships[0];
            return <tr key={member.id} className="border-t border-border/10 align-top"><td className="px-4 py-4"><p className="font-bold">{member.firstName} {member.lastName}</p><p className="text-xs text-muted">{member.mobile}</p></td><td className="px-4 py-4"><p>{membership?.plan.title ?? 'بدون پلن'}</p>{membership && <Badge tone={membership.status === 'ACTIVE' ? 'success' : 'warning'}>{MEMBERSHIP_LABEL[membership.status] ?? membership.status}</Badge>}</td><td className="px-4 py-4">{insurance ? <div><Badge tone={insurance.status === 'APPROVED' ? 'success' : insurance.status === 'REJECTED' ? 'danger' : 'warning'}>{INSURANCE_LABEL[insurance.status] ?? insurance.status}</Badge>{insurance.provider && <p className="mt-1 text-xs text-muted">{insurance.provider}</p>}</div> : <Badge tone="danger">ثبت نشده</Badge>}</td><td className="px-4 py-4">{member.isRestricted ? <Badge tone="warning">محدود</Badge> : <Badge tone="success">کامل</Badge>}</td><td className="px-4 py-4"><div className="flex flex-wrap gap-2">{insurance?.status === 'PENDING' && <><Button size="sm" disabled={busyId === insurance.id} onClick={() => reviewInsurance(member.id, insurance.id, 'APPROVED')}><Check className="size-3.5" />تایید بیمه</Button><Button size="sm" variant="danger" disabled={busyId === insurance.id} onClick={() => reviewInsurance(member.id, insurance.id, 'REJECTED')}><X className="size-3.5" />رد</Button></>}{member.isRestricted && member.parentalConsent && <Button size="sm" disabled={busyId === member.id} onClick={() => approveConsent(member.id)}><ShieldCheck className="size-3.5" />تایید رضایت‌نامه</Button>}{membership?.status === 'PENDING_PAYMENT' && <Button size="sm" onClick={() => setPaymentId(paymentId === membership.id ? null : membership.id)}><CreditCard className="size-3.5" />ثبت پرداخت</Button>}</div>{paymentId === membership?.id && <div className="mt-3 flex items-center gap-2 rounded-xl bg-surface-raised p-3"><select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value as 'CASH' | 'POS')} className="h-9 rounded-lg border border-white/10 bg-surface px-2"><option value="POS">کارت‌خوان</option><option value="CASH">نقدی</option></select><span className="text-xs text-muted">{Number(membership.plan.price).toLocaleString('fa-IR')} تومان</span><Button size="sm" disabled={busyId === membership.id} onClick={() => recordPayment(member)}>تایید پرداخت</Button></div>}</td></tr>;
          })}</tbody>
        </table>
      </div>
    </div>
